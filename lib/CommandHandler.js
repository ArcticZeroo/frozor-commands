...
class CommandHandler{
    ...
    register(...commands){
        for (const command of commands) {
            ...
            this._add(command.name, command);
        ...
        if (typeof input === 'string') {
            try {
                input = await new Promise((resolve, reject)=>{
                    // If given an absolute path, uh... load its files and load the commands I guess.
                    fs.readdir(input, (err, files)=>{
                        ...
                        for (const file of files) {
                            ...
                            const commands = this._create(commandFile, ...d);
    ...
    async process(message, extra = {}, client = this.client) {
        ...
        if (this.commands.has(message.commandName)) {
            ...
            if (message.args.length < command.minArgs) {
                ...
                return notify(this.formatter.minargs);
            }

            if (message.args.length > command.maxArgs) {
                log(false);

                return notify(this.formatter.maxargs);
            }
            ...
            if (!canRun) {
                ...
                return notify(this.formatter.permission);
            }
    ...
    getHelpStatement (formatter = CommandHandler.defaultHelpFormatter, filter = CommandHandler.defaultHelpFilter) {
        ...
        this.commands.entries()
            ...
            .forEach((entry)=>{
                ...
                // If the formatter returns false
                if (!format) {
                    return;
                }
